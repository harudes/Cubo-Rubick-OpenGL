...
void RubickCube::getCenters(glm::vec3 cube[3][3][3]) {
	glm::vec3 sizeDistribution[3][3][3];
	getSizes(sizeDistribution);

	for (int i = 0; i < 3; ++i) {
		for (int j = 0; j < 3; ++j) {
			for (int k = 0; k < 3; ++k) {
				cube[i][j][k].x = sizeDistribution[i][j][k].x / 2;
				if (i > 0)
					cube[i][j][k].x += cube[i - 1][j][k].x + sizeDistribution[i - 1][j][k].x / 2 + offset;

				cube[i][j][k].y = sizeDistribution[i][j][k].y / 2;
				if (j > 0)
					cube[i][j][k].y += cube[i][j - 1][k].y + sizeDistribution[i][j - 1][k].y / 2 + offset;

				cube[i][j][k].z = sizeDistribution[i][j][k].z / 2;
				if (k > 0)
					cube[i][j][k].z += cube[i][j][k - 1].z + sizeDistribution[i][j][k - 1].z / 2 + offset;
			}
		}
	}
}

void RubickCube::getSizes(glm::vec3 cube[3][3][3]) {
	for (int i = 0; i < 3; ++i) {
		for (int j = 0; j < 3; ++j) {
			cube[0][i][j].x = 0.2 * 3 * arista;
		}
	}
	for (int i = 0; i < 3; ++i) {
		for (int j = 0; j < 3; ++j) {
			cube[1][i][j].x = 0.35 * 3 * arista;
		}
	}
	for (int i = 0; i < 3; ++i) {
		for (int j = 0; j < 3; ++j) {
			cube[2][i][j].x = 0.45 * 3 * arista;
		}
	}
	...
	for (int i = 0; i < 3; ++i) {
		for (int j = 0; j < 3; ++j) {
			cube[i][0][j].y = 0.5 * 3 * arista;
		}
	}
	for (int i = 0; i < 3; ++i) {
		for (int j = 0; j < 3; ++j) {
			cube[i][1][j].y = 0.35 * 3 * arista;
		}
	}
	for (int i = 0; i < 3; ++i) {
		for (int j = 0; j < 3; ++j) {
			cube[i][2][j].y = 0.15 * 3 * arista;
		}
	}
	...
	for (int i = 0; i < 3; ++i) {
		for (int j = 0; j < 3; ++j) {
			cube[i][j][0].z = 0.3 * 3 * arista;
		}
	}
	for (int i = 0; i < 3; ++i) {
		for (int j = 0; j < 3; ++j) {
			cube[i][j][1].z = 0.3 * 3 * arista;
		}
	}
	for (int i = 0; i < 3; ++i) {
		for (int j = 0; j < 3; ++j) {
			cube[i][j][2].z = 0.4 * 3 * arista;
		}
	}
}
...
void RubickCube::generateCube(){
	...
	cubitos[1] = new Cube(centers[1][1][1], sizeDistribution[1][1][1] , auxColors, getTextureCoords(auxColors, { 0,0,0,0,0,0 }), shaderProgram, texture);//centro
	updateColors(auxColors, { BLACK,BLACK,BLACK,BLACK,getCharColor(colors[getDuplePosition("B5")]),BLACK });
	cubitos[2] = new Cube(centers[0][1][1], sizeDistribution[0][1][1], auxColors, getTextureCoords(auxColors, { 0,0,0,0,5,0 }), shaderProgram, texture);//B5
	...
	cubitos[3] = new Cube(centers[2][0][1], sizeDistribution[2][0][1], auxColors, getTextureCoords(auxColors, { 0,0,6,0,0,4 }), shaderProgram, texture);//F4, L6
	updateColors(auxColors, { BLACK,BLACK,getCharColor(colors[getDuplePosition("L5")]),BLACK,BLACK,BLACK });
	cubitos[4] = new Cube(centers[1][0][1], sizeDistribution[1][0][1], auxColors, getTextureCoords(auxColors, { 0,0,5,0,0,0 }), shaderProgram, texture);//L5
	...
	cubitos[5] = new Cube(centers[0][0][1], sizeDistribution[0][0][1], auxColors, getTextureCoords(auxColors, { 0,0,4,0,6,0 }), shaderProgram, texture);//B6, L4
	...
	cubitos[6] = new Cube(centers[2][2][1], sizeDistribution[2][2][1], auxColors, getTextureCoords(auxColors, { 0,0,0,4,0,6 }), shaderProgram, texture);//F6, R4
	updateColors(auxColors, { BLACK,BLACK,BLACK,getCharColor(colors[getDuplePosition("R5")]),BLACK,BLACK });
	cubitos[7] = new Cube(centers[1][2][1], sizeDistribution[1][2][1], auxColors, getTextureCoords(auxColors, { 0,0,0,5,0,0 }), shaderProgram, texture);//R5
	...
	cubitos[8] = new Cube(centers[0][2][1], sizeDistribution[0][2][1], auxColors, getTextureCoords(auxColors, { 0,0,0,6,4,0 }), shaderProgram, texture);//R6, B4
	...
	cubitos[9] = new Cube(centers[2][0][2], sizeDistribution[2][0][2], auxColors, getTextureCoords(auxColors, { 0,7,3,0,0,1 }), shaderProgram, texture);//F1, L3, U7
	...
	cubitos[10] = new Cube(centers[1][0][2], sizeDistribution[1][0][2], auxColors, getTextureCoords(auxColors, { 0,4,2,0,0,0 }), shaderProgram, texture);//L2, U4
	...
	cubitos[11] = new Cube(centers[0][0][2], sizeDistribution[0][0][2], auxColors, getTextureCoords(auxColors, { 0,1,1,0,3,0 }), shaderProgram, texture);//B3, L1, U1
	...
	cubitos[17] = new Cube(centers[0][2][2], sizeDistribution[0][2][2], auxColors, getTextureCoords(auxColors, { 0,3,0,3,1,0 }), shaderProgram, texture);//R3, B1, U3
	...
	cubitos[18] = new Cube(centers[2][0][0], sizeDistribution[2][0][0], auxColors, getTextureCoords(auxColors, { 1,0,9,0,0,7 }), shaderProgram, texture);//F7, L9, D1
	...
	cubitos[19] = new Cube(centers[1][0][0], sizeDistribution[1][0][0], auxColors, getTextureCoords(auxColors, { 4,0,8,0,0,0 }), shaderProgram, texture);//L8, D2
	...
	cubitos[20] = new Cube(centers[0][0][0], sizeDistribution[0][0][0], auxColors, getTextureCoords(auxColors, { 7,0,7,0,9,0 }), shaderProgram, texture);//L7, B9, D3
	...
	cubitos[21] = new Cube(centers[2][1][0], sizeDistribution[2][1][0], auxColors, getTextureCoords(auxColors, { 2,0,0,0,0,8 }), shaderProgram, texture);//F8, D2
	updateColors(auxColors, { getCharColor(colors[getDuplePosition("D5")]),BLACK,BLACK,BLACK,BLACK,BLACK });
	cubitos[22] = new Cube(centers[1][1][0], sizeDistribution[1][1][0], auxColors, getTextureCoords(auxColors, { 5,0,0,0,0,0 }), shaderProgram, texture);//D5
	...
	cubitos[23] = new Cube(centers[0][1][0], sizeDistribution[0][1][0], auxColors, getTextureCoords(auxColors, { 8,0,0,0,8,0 }), shaderProgram, texture);//B8, D2
	...
	cubitos[24] = new Cube(centers[2][2][0], sizeDistribution[2][2][0], auxColors, getTextureCoords(auxColors, { 3,0,0,7,0,9 }), shaderProgram, texture);//F9, R7, D3
	...
	cubitos[25] = new Cube(centers[1][2][0], sizeDistribution[1][2][0], auxColors, getTextureCoords(auxColors, {6,0,0,8,0,0 }), shaderProgram, texture);//D6, R8
	...
	cubitos[26] = new Cube(centers[0][2][0], sizeDistribution[0][2][0], auxColors, getTextureCoords(auxColors, { 9,0,0,9,7,0 }), shaderProgram, texture);//R9, B7, D1
